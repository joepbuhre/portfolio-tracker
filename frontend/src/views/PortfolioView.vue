<template>
    ...
    <div class="flex">
        <div class="w-1/5">
            ...
            <button
                @click="showAccountValues = !showAccountValues"
                class="flex gap-2"
            >
                <EyeOffIcon v-if="showAccountValues" />
                <Eye v-else />
                <p>
                    <span v-if="showAccountValues">Hide</span
                    ><span v-else>Show</span><span>&nbsp;Account values</span>
                </p>
            </button>
            <div v-for="groupby in groupbys">
                <input
                    type="checkbox"
                    v-model="selectGroupbys"
                    :value="groupby.value"
                    :id="groupby.value"
                />
        ...
        <div class="w-full grid grid-cols-1 gap-3" v-if="hasData">
            ...
            <div v-for="dfType in respSorted" class="py-2">
                ...
                <p v-else>Sorry no data has been found</p>
    ...
    if (file.value !== null) {
        ...
        if (main.getUserId !== null) {
            ...
            [...selectGroupbys.value, "description"].forEach((el) => {
                api.post(`/stocks/anonymous/${el}`, formdata).then(
                    (res: AxiosResponse) => {
                        resp.value.push({
                            name: el,
                            data: res.data,
    ...
    Promise.allSettled([
        ...[...selectGroupbys.value].map((el) => {
            return new Promise((resolve, reject) => {
                api.get(`/stocks/${el}`, {
                    headers: { "x-userid": main.getUserId },
                })
...
const getHeaderObject = (dataArr: any): Header => {
    return Object.fromEntries(
        Object.keys(dataArr).map((el) => [
            el,
            {
                name: el,
                formatter:
